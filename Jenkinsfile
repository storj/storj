    ...
    stage('CRDB Run Rolling Upgrade Test') {
        ...
        try {
          ...
          sh '''docker run \
            ...
            -v $PWD:$PWD \
            -w $PWD \
            --entrypoint $PWD/testsuite/rolling-upgrade/start-sim.sh \
            -e BRANCH_NAME \
            -e STORJ_SIM_POSTGRES \
            -e STORJ_SIM_REDIS \
            -e STORJ_MIGRATION_DB \
            -e STORJ_SKIP_FIX_LAST_NETS \
            -e STORJ_CONSOLE_SIGNUP_ACTIVATION_CODE_ENABLED \
            --link redis-$BUILD_NUMBER:redis \
            --link cockroach-$BUILD_NUMBER:cockroach \
            golang:1.25.6'''
        }
        catch(err){
            throw err
