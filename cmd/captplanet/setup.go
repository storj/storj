...
func cmdSetup(cmd *cobra.Command, args []string) (err error) {
	...
	overrides := map[string]interface{}{
		...
		"satellite.server.public-address":                     joinHostPort(setupCfg.ListenHost, startingPort+1),
		"satellite.server.cert-verification.revocation-dburl": "redis://127.0.0.1:6378?db=2&password=abc123",
	...
	for i := 0; i < len(runCfg.StorageNodes); i++ {
		...
		overrides[storagenode+"server.public-address"] = joinHostPort(setupCfg.ListenHost, startingPort+i*2+3)
		overrides[storagenode+"server.cert-verification.revocation-dburl"] = "redis://127.0.0.1:6378?db=2&password=abc123"
