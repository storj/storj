...
2. They are then distributed into queues using every storage node. It will preferentially choose nodes specified in `--service.priority-nodes-path` file, one storagenode id per line.
