...
func TestVerifier(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		var count int
		count, err = service.Verify(ctx, alias0, planet.StorageNodes[0].NodeURL(), nodeVersion,
		...
		const throttleN = 5
		count, err = service.Verify(ctx, alias0, planet.StorageNodes[0].NodeURL(), nodeVersion, validSegments[:throttleN], false)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		count, err = service.Verify(ctx, alias0, planet.StorageNodes[0].NodeURL(), nodeVersion, validSegments, true)
		require.Error(t, err)
...
// TODO maybe merge with TestVerifier
func TestVerifyWithExists(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		SatelliteCount: 1, StorageNodeCount: 4, UplinkCount: 1,
		Reconfigure: testplanet.Reconfigure{
			Satellite: testplanet.ReconfigureRS(4, 4, 4, 4),
		...
		for _, node := range planet.StorageNodes {
			...
			require.True(t, ok)
			g.Go(func() error {
				_, err := service.VerifyWithExists(ctx, alias, node.NodeURL(), nodeVersion, validSegments)
				return err
			})
		}
		require.NoError(t, g.Wait())
		...
		var count int
		count, err = service.VerifyWithExists(ctx, alias0, planet.StorageNodes[0].NodeURL(), nodeVersion,
			[]*segmentverify.Segment{validSegment0, missingSegment, validSegment1})
		require.NoError(t, err)
		require.Equal(t, 3, count)
		require.Equal(t, segmentverify.Status{Found: 1}, validSegment0.Status)
		require.Equal(t, segmentverify.Status{NotFound: 1}, missingSegment.Status)
		...
		err = planet.StopNodeAndUpdate(ctx, planet.StorageNodes[0])
		require.NoError(t, err)
		count, err = service.VerifyWithExists(ctx, alias0, planet.StorageNodes[0].NodeURL(), nodeVersion, validSegments)
		require.Error(t, err)
		require.Equal(t, 0, count)
		require.True(t, segmentverify.ErrNodeOffline.Has(err))
	})
