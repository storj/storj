...
# RUN ls /go/src/storj.io/storj
# ADD scripts/release.sh /go/src/storj.io/storj/cmd/storagenode
# COPY scripts/release.sh /go/src/storj.io/storj/cmd/storagenode/release.sh
WORKDIR /go/src/storj.io/storj/cmd/storagenode

# dependencies + binary
RUN apk add git gcc musl-dev
#RUN unset GOPATH && go mod vendor
RUN alpha/release.sh build -a -installsuffix cgo -o storagenode .
