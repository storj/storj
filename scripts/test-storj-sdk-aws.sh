...
aws s3 --endpoint=$GATEWAY_0_ADDR mb s3://bucket

aws configure set default.s3.multipart_threshold 1TB
aws s3 --endpoint=$GATEWAY_0_ADDR cp $SRC_DIR/small-upload-testfile s3://bucket/small-testfile
aws s3 --endpoint=$GATEWAY_0_ADDR cp $SRC_DIR/big-upload-testfile   s3://bucket/big-testfile
...
aws s3 --endpoint=$GATEWAY_0_ADDR cp $SRC_DIR/multipart-upload-testfile s3://bucket/multipart-testfile

aws s3 --endpoint=$GATEWAY_0_ADDR ls s3://bucket
aws s3 --endpoint=$GATEWAY_0_ADDR cp s3://bucket/small-testfile     $DST_DIR/small-download-testfile
aws s3 --endpoint=$GATEWAY_0_ADDR cp s3://bucket/big-testfile       $DST_DIR/big-download-testfile
aws s3 --endpoint=$GATEWAY_0_ADDR cp s3://bucket/multipart-testfile $DST_DIR/multipart-download-testfile
aws s3 --endpoint=$GATEWAY_0_ADDR rb s3://bucket --force
