...
|               | Low online score                        | Perform GE on node with less than 50% of score. Node should fail to GE                                                                                                                                                            | Covered  |
|               | Too many nodes call GE at the same time | We should transfer all the pieces to available nodes anyway. Example: start with 8 nodes(RS settings 2,3,4,4) and call GE on 4 nodes at the same time                                                                             |          |
...
|               | GE started before feature deployment    | Node should stop transferring new pieces and should be treated by the new rules.                                                                                                                                                  |          |