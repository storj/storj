...
func TestBasic(t *testing.T) {
	...
	test := func(version storj.IDVersion) {
		...
		// ping a satellite
		_, err = planet.StorageNodes[0].Kademlia.Service.Ping(ctx, planet.Satellites[0].Local().Node) // TODO KAD UPDATE
		require.NoError(t, err)

		// ping a storage node
		_, err = planet.StorageNodes[0].Kademlia.Service.Ping(ctx, planet.StorageNodes[1].Local().Node) // TODO KAD UPDATE
		require.NoError(t, err)
