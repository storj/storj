...
<script lang="ts">
    ...
    import { BUCKET_USAGE_ACTIONS, NOTIFICATION_ACTIONS, PM_ACTIONS } from "../../../utils/constants/actionNames";
    ...
    @Component({
		...
		methods: {
        	onPageClick: async function (event: any, page: number) {
				const response = await this.$store.dispatch(PM_ACTIONS.FETCH, page);
			...
			nextPage: async function() {
				...
				const response = await this.$store.dispatch(PM_ACTIONS.FETCH, (this as any).currentPage + 1);
				if (!response.isSuccess) {
					this.$store.dispatch(NOTIFICATION_ACTIONS.ERROR, 'Unable to fetch buckets: ' + response.errorMessage);
				}
			},
		...
		computed: {
        	pages: function(): number[] {
        		return new Array(this.$store.state.projectMembersModule.page.pageCount);
			},
			currentPage: function (): number {
				return this.$store.state.projectMembersModule.page.currentPage;
			},
			firstEdge: function (): number {
				return this.$store.state.projectMembersModule.page.offset + 1;
			},
			lastEdge: function (): number {
        		let offset = this.$store.state.projectMembersModule.page.offset;
				let bucketsLength = this.$store.state.projectMembersModule.page.projectMembers.length;
			...
			isFirstPage: function() {
        		return this.$store.state.projectMembersModule.page.currentPage === 1;
			},
			isLastPage: function (): boolean {
        		let currentPage = this.$store.state.projectMembersModule.page.currentPage;
        		let pageCount = this.$store.state.projectMembersModule.page.pageCount;
    ...
    export default class TeamMemberPaginationArea extends Vue {}
</script>
