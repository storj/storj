...
<template>
    <div class="node-ui-container">
        <Loading
                v-if="isLoading" />
        <div class="node-ui-container__header">
            <svg width="51" height="57" viewBox="0 0 54 60" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="node-ui-container__main">
            <div class="node-ui-container__main__text-container">
                <h1>Storj Node</h1>
                <h2>Connectivity Check</h2>
            </div>
            <div class="node-ui-container__main__input-container">
                <input
                        class="node-ui-container__main__input-container__input"
                        placeholder="Enter your Node ID"
                        v-model="nodeId">
                <div class="node-ui-container__main__input-container__button" v-on:click="checkAvailability">
                    <p class="node-ui-container__main__input-container__button__text">Look up</p>
                </div>
            </div>
        </div>
        <div class="node-ui-container__footer">
            <p>This service can help you to check connectivity status of your Storj Node with Kademlia</p>
        </div>
    </div>
</template>
...
<script lang="ts">
    ...
    @Component({
        ...
        methods: {
            checkAvailability: async function(): Promise<any> {
                this.$store.dispatch(ACTIONS.SET_LOADING);
                let id = this.$data.nodeId.trim();

                setTimeout(() => this.$store.dispatch(ACTIONS.CHECK_NODE_STATUS, id), 1000);
            },
        },
        computed: {
    ...
    .node-ui-container {
        ...
        &__main {
            ...
            &__input-container {
                width: 838px;
                height: 76px;
                box-shadow: 0 7px 17px 0 #111a2b;
                border-radius: 12px;
                position: relative;
                margin-top: 64px;
                ...
                &__button {
                    ...
                    &:hover {
                        cursor: pointer;
                        box-shadow: 0 7px 17px -4px #608edc;
