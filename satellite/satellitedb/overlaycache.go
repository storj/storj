...
func updateReputation(newStatus bool, alpha, beta, lambda, w float64, totalCount int64) (int64, float64, float64) {
	totalCount++
	var v float64 = 1
	if newStatus {
		v = -1
	}
	new_alpha = lambda*alpha + w*(1+v)/2
	new_beta = lambda*beta + w*(1-v)/2
