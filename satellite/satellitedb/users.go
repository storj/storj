...
func (users *users) GetAllByEmail(ctx context.Context, email string) (_ []*console.User, err error) {
	defer mon.Task()(&ctx)(&err)

	allUsers, err := users.db.All_User_By_Email(ctx, dbx.User_Email(email))
	if err != nil {
		return nil, err
	}

	return usersFromDbxSlice(ctx, allUsers)
}
...
func usersFromDbxSlice(ctx context.Context, usersDbx []*dbx.User) (_ []*console.User, err error) {
	defer mon.Task()(&ctx)(&err)
	...
	for _, userDbx := range usersDbx {
		user, err := userFromDBX(ctx, userDbx)
		if err != nil {
	...
	return users, errs.Combine(errors...)
}
