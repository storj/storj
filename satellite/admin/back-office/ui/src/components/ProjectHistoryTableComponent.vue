    ...
    <v-card variant="flat" :border="true" rounded="xlg">
        ...
        >
            ...
            <template #item.userID="{ item }">
                <v-chip v-if="item.userID" variant="tonal" size="small" rounded="lg" @click="goToUser(item.userID)">
                    {{ item.userID }}
                </v-chip>
                <template v-else>
                    —
...
import { ChangeLog, Project } from '@/api/client.gen';
...
function fetchHistory() {
    withLoading(async () => {
        try {
            history.value = await projectsStore.getHistory(props.project.id, exact.value);
        } catch (error) {
            notify.error(error);
        }
    });
...
watch(() => props.project, () => fetchHistory());
</script>
