...
func TestAdminBucketPlacementAPI(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		for _, testCase := range testCases {
			baseURL := fmt.Sprintf("http://%s/legacy/api/projects/%s/buckets/%s", address, testCase.project, string(testCase.bucket))
			t.Log(baseURL)
			basePlacementURL := fmt.Sprintf("http://%s/legacy/api/projects/%s/buckets/%s/placement", address, testCase.project, string(testCase.bucket))
...
func TestAdminUpdateBucketValueAttributionPlacement(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		baseURL := fmt.Sprintf("http://%s/legacy/api/projects/%s/buckets/%s/value-attributions", address, project.ID, bucketName)
