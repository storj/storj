...
func TestAdminProjectPlacementAPI(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		for _, testCase := range testCases {
			baseURL := fmt.Sprintf("http://%s/legacy/api/projects/%s", address, testCase.project)
			t.Log(baseURL)
			basePlacementURL := fmt.Sprintf("http://%s/legacy/api/projects/%s/placement", address, testCase.project)
