...
func runTests(t *testing.T, ctx *testcontext.Context, planet *testplanet.Planet) {
	...
	t.Run("create with default expiration", func(t *testing.T) {
		...
		req, err := http.NewRequestWithContext(ctx, http.MethodPost, fmt.Sprintf("http://"+address.String()+"/legacy/api/restkeys/%s", user.Email), body)
		require.NoError(t, err)
	...
	t.Run("revoke key", func(t *testing.T) {
		...
		req, err := http.NewRequestWithContext(ctx, http.MethodPut, fmt.Sprintf("http://"+address.String()+"/legacy/api/restkeys/%s/revoke", apiKey), nil)
		require.NoError(t, err)
		req.Header.Set("Authorization", sat.Config.Console.AuthToken)
