...
func TestApiKeyAdd(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		req, err := http.NewRequestWithContext(
			ctx,
			http.MethodPost,
			fmt.Sprintf("http://"+address.String()+"/legacy/api/projects/%s/apikeys", projectID.String()),
			body,
		)
		require.NoError(t, err)
...
func TestApiKeyDelete(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		link := fmt.Sprintf("http://"+address.String()+"/legacy/api/apikeys/%s", apikey)
...
func TestApiKeyDelete_ByName(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		link := fmt.Sprintf("http://"+address.String()+"/legacy/api/projects/%s/apikeys", projectID.String())
		body := assertReq(
			ctx,
			t,
			link,
...
func TestApiKeysList(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		link := "http://" + address.String() + "/legacy/api/projects/" + project.ID.String() + "/apikeys"
...
func TestAPIKeyManagementGet(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		address := planet.Satellites[0].Admin.Admin.Listener.Addr()
		link := fmt.Sprintf("http://"+address.String()+"/legacy/api/apikeys/%s", apiKey.Serialize())
