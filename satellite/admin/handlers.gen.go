...
type UserManagementService interface {
	...
	ToggleInactivityExemption(ctx context.Context, authInfo *AuthInfo, userID uuid.UUID, request ToggleInactivityExemptionRequest) api.HTTPError
...
func NewSettings(log *zap.Logger, mon *monkit.Scope, service SettingsService, router *mux.Router, auth *Authorizer) *SettingsHandler {
	...
	settingsRouter := router.PathPrefix("/api/v1/settings").Subrouter()
...
func NewPlacementManagement(log *zap.Logger, mon *monkit.Scope, service PlacementManagementService, router *mux.Router) *PlacementManagementHandler {
	...
	placementsRouter := router.PathPrefix("/api/v1/placements").Subrouter()
...
func NewProductManagement(log *zap.Logger, mon *monkit.Scope, service ProductManagementService, router *mux.Router) *ProductManagementHandler {
	...
	productsRouter := router.PathPrefix("/api/v1/products").Subrouter()
...
func NewUserManagement(log *zap.Logger, mon *monkit.Scope, service UserManagementService, router *mux.Router, auth *Authorizer) *UserManagementHandler {
	...
	usersRouter.HandleFunc("/{userID}/inactivity-exemption", handler.handleToggleInactivityExemption).Methods("PUT")
...
func NewProjectManagement(log *zap.Logger, mon *monkit.Scope, service ProjectManagementService, router *mux.Router, auth *Authorizer) *ProjectManagementHandler {
	...
	projectsRouter := router.PathPrefix("/api/v1/projects").Subrouter()
...
func (h *UserManagementHandler) handleToggleInactivityExemption(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	var err error
	defer h.mon.Task()(&ctx)(&err)
	...
	payload := ToggleInactivityExemptionRequest{}
	if err = json.NewDecoder(r.Body).Decode(&payload); err != nil {
		api.ServeError(h.log, w, http.StatusBadRequest, err)
		return
	}
	...
	if h.auth.IsRejected(w, r, 70368744177664) {
		return
	}

	httpErr := h.service.ToggleInactivityExemption(ctx, authInfo, userID, payload)
	if httpErr.Err != nil {
		api.ServeError(h.log, w, httpErr.Status, httpErr.Err)
	}
}
