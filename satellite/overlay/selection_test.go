...
func TestFindStorageNodesDistinctNetworks(t *testing.T) {
	...
	testplanet.Run(t, testplanet.Config{
		...
		n2, err = satellite.Overlay.Service.SelectionCache.GetNodes(ctx, req)
		require.Error(t, err)
		// GetNodes returns 1 more node than FindStorageNodesWithPreferences because the way the queries are
		// FindStorageNodesWithPreferences gets the IPs for the excludedNodes and excludes all the IPs as well
		// where the selection cache only filters IPs at time of selection and includes one of the nodes with a duplicate IP
