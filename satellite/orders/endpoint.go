...
type DB interface {
	...
	CreateSerialInfo(ctx context.Context, serialNumber storj.SerialNumber, bucketID paths.BucketID, limitExpiration time.Time) error
	...
	UpdateBucketBandwidthAllocation(ctx context.Context, bucketID paths.BucketID, action pb.PieceAction, amount int64, intervalStart time.Time) error
	// UpdateBucketBandwidthSettle updates 'settled' bandwidth for given bucket
	UpdateBucketBandwidthSettle(ctx context.Context, bucketID paths.BucketID, action pb.PieceAction, amount int64, intervalStart time.Time) error
	// UpdateBucketBandwidthInline updates 'inline' bandwidth for given bucket
	UpdateBucketBandwidthInline(ctx context.Context, bucketID paths.BucketID, action pb.PieceAction, amount int64, intervalStart time.Time) error
	...
	GetBucketBandwidth(ctx context.Context, bucketID paths.BucketID, from, to time.Time) (int64, error)
...
func (endpoint *Endpoint) Settlement(stream pb.Orders_SettlementServer) (err error) {
	...
	for {
		...
		bucketID, err := paths.ParseBucketID(rawBucketID)
		if err != nil {
