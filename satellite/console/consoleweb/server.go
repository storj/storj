...
func NewServer(logger *zap.Logger, config Config, service *console.Service, mailService *mailservice.Service, listener net.Listener) *Server {
	...
	authRouter.HandleFunc("/", authController.Register).Methods(http.MethodPost)
	authRouter.Handle("/change-password", server.withAuth(http.HandlerFunc(authController.ChangePassword))).Methods(http.MethodPatch)
	authRouter.Handle("/{password}", server.withAuth(http.HandlerFunc(authController.Delete))).Methods(http.MethodDelete)
	...
	authRouter.HandleFunc("/forgot-password/{email}", authController.ForgotPassword).Methods(http.MethodPut)
	authRouter.HandleFunc("/resend-email/{id}", authController.ResendEmail).Methods(http.MethodPut)
