...
// UpdateInvoiceStatus will update all open invoices within the specified date range to the requested status
func (service *Service) UpdateInvoiceStatus(ctx context.Context, startPeriod, endPeriod time.Time, status string, dryRun bool) (err error) {
	defer mon.Task()(&ctx)(&err)

	switch {
	case strings.EqualFold(status, string(stripe.InvoiceStatusUncollectible)):
		service.updateInvoiceStatus(ctx, startPeriod, endPeriod, func(invoiceId string) error {
	...
	case strings.EqualFold(status, string(stripe.InvoiceStatusVoid)):
		service.updateInvoiceStatus(ctx, startPeriod, endPeriod, func(invoiceId string) error {
	...
	case strings.EqualFold(status, string(stripe.InvoiceStatusPaid)):
		service.updateInvoiceStatus(ctx, startPeriod, endPeriod, func(invoiceId string) error {
...
func (service *Service) updateInvoiceStatus(ctx context.Context, startPeriod, endPeriod time.Time, updateStatus func(string) error) (err error) {
	defer mon.Task()(&ctx)(&err)
