...
func (jq *jobQueue) Pop() any {
	...
	if jq.unmarkingError == nil {
		jq.highWater, jq.unmarkingError = markUnused(jq.mem, len(jq.priorityHeap)-1, jq.highWater, jq.memReleaseThreshold)
	}
...
func (jq *jobQueue) Truncate() {
	...
	if jq.unmarkingError == nil {
		jq.highWater, jq.unmarkingError = markUnused(jq.mem, 0, jq.highWater, jq.memReleaseThreshold)
	}
}
...
func (jq *jobQueue) cleanQueue(updatedBefore time.Time) (removed int) {
	...
	if jq.unmarkingError == nil {
		jq.highWater, jq.unmarkingError = markUnused(jq.mem, len(jq.priorityHeap), jq.highWater, jq.memReleaseThreshold)
	}
