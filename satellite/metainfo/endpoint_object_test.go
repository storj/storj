...
func TestEndpoint_ListObjectsMetadata(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		check := func(inc pb.ObjectListItemIncludes, expected []expect) {
			...
			t.Run(name, func(t *testing.T) {
				...
				for i, exp := range expected {
					...
					shouldHaveKey := exp.etag || exp.metadata // when the output contains etag or metadata
					...
					assert.Equal(t, exp.metadata, len(resp.Items[i].EncryptedMetadata) > 0, name)
				}
			})
		}
		...
		check(pb.ObjectListItemIncludes{
			...
			{metadata: true, etag: true},
			{metadata: true, etag: false},
			{metadata: true, etag: true}, // this includes metadata generated by the satellite, which contains segment count
			{metadata: false, etag: false},
		})
		...
		check(pb.ObjectListItemIncludes{
			...
			{metadata: true, etag: false},
			{metadata: true, etag: false},
			{metadata: true, etag: false}, // this includes metadata generated by the satellite, which contains segment count
			{metadata: false, etag: false},
		})
