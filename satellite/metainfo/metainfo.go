...
func NewEndpoint(log *zap.Logger, pointerdb *pointerdb.Service, allocation *pointerdb.AllocationSigner, statDB statdb.DB, apiKeys APIKeys, signer signing.Signer, selectionPreferences *statdb.NodeSelectionConfig) *Endpoint {
...
func (endpoint *Endpoint) CreateSegment(ctx context.Context, req *pb.SegmentWriteRequest) (resp *pb.SegmentWriteResponse, err error) {
	...
	criteria := &statdb.NodeCriteria{
		FreeBandwidth:      maxPieceSize,
		FreeDisk:           maxPieceSize,
		AuditCount:         endpoint.selectionPreferences.AuditCount,
		AuditSuccessRatio:  endpoint.selectionPreferences.AuditSuccessRatio,
		UptimeCount:        endpoint.selectionPreferences.UptimeCount,
		UptimeSuccessRatio: endpoint.selectionPreferences.UptimeRatio,
	}
	nodes, err := endpoint.statDB.SelectStorageNodes(ctx, redundancy.TotalCount(), criteria)
	if err != nil {
		return nil, status.Errorf(codes.Internal, err.Error())
	}
...
func (endpoint *Endpoint) createOrderLimitsForSegment(ctx context.Context, pointer *pb.Pointer, action pb.PieceAction) ([]*pb.AddressedOrderLimit, error) {
	...
	for _, piece := range pointer.GetRemote().GetRemotePieces() {
		...
		node, err := endpoint.statDB.Get(ctx, piece.NodeId)
		if err != nil {
			return nil, err
		}
