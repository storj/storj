...
func (verifier *Verifier) Verify(ctx context.Context, path storj.Path, skip map[storj.NodeID]bool) (report *Report, err error) {
	...
	if len(offlineNodes) > 0 {
		verifier.log.Debug("order limits not created for some nodes (offline/disqualified)", zap.String("Segment Path", path), zap.Strings("Node IDs", offlineNodes.Strings()))
	}
	...
	for pieceNum, share := range shares {
		...
		if transport.Error.Has(share.Error) {
			if errs.Is(share.Error, context.DeadlineExceeded) {
				...
				verifier.log.Debug("dial timeout (offline)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
				continue
			}
			if errs2.IsRPC(share.Error, codes.Unknown) {
				// dial failed -- offline node
				offlineNodes = append(offlineNodes, share.NodeID)
				verifier.log.Debug("dial failed (offline)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
				continue
			}
			// unknown transport error
			containedNodes[pieceNum] = share.NodeID
			verifier.log.Debug("unknown transport error (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
		}

		if errs2.IsRPC(share.Error, codes.NotFound) {
			// missing share
			failedNodes = append(failedNodes, share.NodeID)
			verifier.log.Debug("piece not found (audit failed)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
			continue
		}
		...
		if errs2.IsRPC(share.Error, codes.DeadlineExceeded) {
			...
			containedNodes[pieceNum] = share.NodeID
			verifier.log.Debug("download timeout (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
			continue
		}

		// unknown error
		containedNodes[pieceNum] = share.NodeID
		verifier.log.Debug("unknown error (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", share.NodeID), zap.Error(share.Error))
	}
...
func (verifier *Verifier) Reverify(ctx context.Context, path storj.Path) (report *Report, err error) {
	...
	for _, piece := range pieces {
		...
		if err != nil {
			...
			verifier.log.Debug("error getting from containment db", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
			continue
		}
		containedInSegment++
		...
		go func(pending *PendingAudit, piece *pb.RemotePiece) {
			...
			if err != nil {
				if overlay.ErrNodeDisqualified.Has(err) {
					...
					ch <- result{nodeID: piece.NodeId, status: erred, err: err}
					verifier.log.Debug("order limit not created (disqualified)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId))
					return
				}
				if overlay.ErrNodeOffline.Has(err) {
					ch <- result{nodeID: piece.NodeId, status: offline}
					verifier.log.Debug("order limit not created (offline)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId))
					return
				}
				ch <- result{nodeID: piece.NodeId, status: erred, err: err}
				verifier.log.Debug("error creating order limit", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
				return
			}
			...
			if getErr != nil {
				if ErrContainedNotFound.Has(getErr) {
					ch <- result{nodeID: piece.NodeId, status: skipped}
					verifier.log.Debug("pending audit deleted during reverification", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(getErr))
					return
				}
				ch <- result{nodeID: piece.NodeId, status: erred, err: getErr}
				verifier.log.Debug("error getting from containment db", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(getErr))
				return
			}
			...
			if err != nil {
				if transport.Error.Has(err) {
					if errs.Is(err, context.DeadlineExceeded) {
						// dial timeout
						ch <- result{nodeID: piece.NodeId, status: offline}
						verifier.log.Debug("dial timeout (offline)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
						return
					}
					if errs2.IsRPC(err, codes.Unknown) {
						// dial failed -- offline node
						verifier.log.Debug("dial failed (offline)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
						ch <- result{nodeID: piece.NodeId, status: offline}
						return
					}
					// unknown transport error
					ch <- result{nodeID: piece.NodeId, status: contained, pendingAudit: pending}
					verifier.log.Debug("unknown transport error (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
					return
				}
				if errs2.IsRPC(err, codes.NotFound) {
					...
					if err != nil {
						...
						verifier.log.Debug("audit source deleted before reverification", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
						return
					}
					// remove failed audit pieces from the pointer so as to only penalize once for failed audits
					err = verifier.removeFailedPieces(ctx, pending.Path, oldPtr, storj.NodeIDList{pending.NodeID})
					if err != nil {
						verifier.log.Warn("failed to delete failed pieces", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
					}
					// missing share
					ch <- result{nodeID: piece.NodeId, status: failed}
					verifier.log.Debug("piece not found (audit failed)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
					return
				}
				if errs2.IsRPC(err, codes.DeadlineExceeded) {
					// dial successful, but download timed out
					ch <- result{nodeID: piece.NodeId, status: contained, pendingAudit: pending}
					verifier.log.Debug("download timeout (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
					return
				}
				// unknown error
				ch <- result{nodeID: piece.NodeId, status: contained, pendingAudit: pending}
				verifier.log.Debug("unknown error (contained)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId), zap.Error(err))
				return
			}
			downloadedHash := pkcrypto.SHA256Hash(share.Data)
			if bytes.Equal(downloadedHash, pending.ExpectedShareHash) {
				ch <- result{nodeID: piece.NodeId, status: success}
				verifier.log.Debug("hashes match (audit success)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId))
			} else {
				oldPtr, err := verifier.checkIfSegmentAltered(ctx, pending.Path, pointer)
				if err != nil {
					ch <- result{nodeID: piece.NodeId, status: success}
				...
				verifier.log.Debug("hashes mismatch (audit failed)", zap.String("Segment Path", path), zap.Stringer("Node ID", piece.NodeId),
