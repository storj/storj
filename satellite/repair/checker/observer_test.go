...
func BenchmarkRemoteSegment(b *testing.B) {
	testplanet.Bench(b, testplanet.Config{
		...
		observer := checker.NewObserver(zap.NewNop(), planet.Satellites[0].DB.RepairQueue(),
			planet.Satellites[0].Auditor.Overlay, nodeselection.NewPlacementDefinitions().CreateFilters, planet.Satellites[0].Config.Checker)
	...
	testplanet.Run(t, testplanet.Config{
		...
		for i, tc := range []testCase{
			...
			t.Run("#"+strconv.Itoa(i), func(t *testing.T) {
				...
				ok, err := allPiecesInPlacement(ctx, planet.Satellites[0].Overlay.Service, segments[0].Pieces, nodeselection.NewPlacementDefinitions().CreateFilters(segments[0].Placement))
				require.NoError(t, err)
				require.False(t, ok)
