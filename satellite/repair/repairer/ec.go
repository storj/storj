...
func (ec *ECRepairer) Get(ctx context.Context, limits []*pb.AddressedOrderLimit, privateKey storj.PiecePrivateKey, es eestream.ErasureScheme, dataSize int64) (_ io.ReadCloser, _ storj.NodeIDList, err error) {
	...
	var (
		successfulPieces, inProgress int
		failedNodeIDs                storj.NodeIDList
	)
			...
			for {
				...
				if err != nil {
					...
					if ErrPieceHashVerifyFailed.Has(err) {
						failedNodeIDs = append(failedNodeIDs, limit.GetLimit().StorageNodeId)
					} else {
	...
	return decodeReader, failedNodeIDs, nil
}
