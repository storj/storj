...
// readTimestamp, when non-zero, makes the scan read a consistent snapshot of the
// database at that time. It is generated by the caller when the run starts (from
// the pinned GC safepoint on TiDB) rather than configured, so that it is always a
// timestamp the backing store is guaranteed to still retain.
func NewGarbageCollectionBF(log *zap.Logger, db DB, metabaseDB *metabase.DB, revocationDB extensions.RevocationDB,
	versionInfo version.Info, config *Config, atomicLogLevel *zap.AtomicLevel, readTimestamp time.Time) (*GarbageCollectionBF, error) {
	...
	{ // setup garbage collection bloom filters
		...
		provider := rangedloop.NewMetabaseRangeSplitterWithReadTimestamp(log.Named("rangedloop-metabase-range-splitter"),
			metabaseDB, config.RangedLoop, readTimestamp)
