...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --extra-tags)
      if [[ "$ACTION" != "push" ]]; then
        echo "Error: --extra-tags can only be used with push."
        exit 1
      fi
...
if [[ "$ACTION" == "push" ]]; then
  # ensure the version tag is included in extra tags if unified-tag is enabled
  if [[ "$UNIFIED_TAG" == "true" ]]; then
    EXTRA_TAGS+=("$VERSION")
  fi

  # add latest if the flag is set
