...
func (peStore *PieceExpirationStore) GetLatestExpired(ctx context.Context, satellite storj.NodeID, before time.Time) (string, time.Time, error) {
	...
	if err != nil {
		...
		return "", time.Time{}, ErrPieceExpiration.Wrap(err)
	}
	defer func() { _ = dirfd.Close() }()
	names, err := dirfd.Readdirnames(-1)
	if err != nil {
	...
	for _, name := range names {
		...
		if hour.Before(before) {
			return filepath.Join(peDir, name), hour.UTC().Truncate(time.Hour), nil
		}
	}
	return "", time.Time{}, nil
}
