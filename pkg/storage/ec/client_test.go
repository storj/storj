...
TestLoop:
	for i, tt := range []struct {
		...
		for _, n := range tt.nodes {
			...
			gomock.InOrder(
				ps.EXPECT().Put(gomock.Any(), derivedID, gomock.Any(), ttl, &pb.FundsOrder{}).Return(errs[n]).
					Do(func(ctx context.Context, id psclient.PieceID, data io.Reader, ttl time.Time, ba *pb.FundsOrder) {
		...
		successfulNodes, err := ec.Put(ctx, tt.nodes, rs, id, r, ttl, &pb.FundsOrder{})
