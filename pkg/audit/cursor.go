...
func NewCursor(pointerdb *pointerdb.Service, allocation *pointerdb.AllocationSigner, statDB statdb.DB, identity *identity.FullIdentity) *Cursor {
	return &Cursor{
		pointerdb:  pointerdb,
...
func (cursor *Cursor) createOrderLimits(ctx context.Context, pointer *pb.Pointer) ([]*pb.AddressedOrderLimit, error) {
	...
	for _, piece := range pointer.GetRemote().GetRemotePieces() {
		...
		node, err := cursor.statDB.Get(ctx, piece.NodeId)
		if err != nil {
			return nil, err
		}
