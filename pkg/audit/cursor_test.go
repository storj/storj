...
func TestAuditSegment(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		tests, cursor, metainfoService := populateTestData(t, planet, time.Now().Add(3*time.Second))
		...
		t.Run("probabilisticTest", func(t *testing.T) {
			list, _, err := metainfoService.List(ctx, "", "", "", true, 10, meta.None)
...
func TestDeleteExpired(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		_, cursor, metainfoService := populateTestData(t, planet, time.Now().Add(-time.Second*1))
...
func populateTestData(t *testing.T, planet *testplanet.Planet, expiration time.Time) ([]testData, *audit.Cursor, *metainfo.Service) {
	...
	cursor := audit.NewCursor(metainfoService)
	...
	t.Run("putToDB", func(t *testing.T) {
		for _, tt := range tests {
			...
			t.Run(test.bm, func(t *testing.T) {
				...
				require.NoError(t, metainfoService.Put(ctx, test.path, pointer))
			})
		}
	})
	return tests, cursor, metainfoService
}
