...
func TestAuditSegment(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		tests, cursor, mi := populateTestData(t, planet, time.Now().Add(3*time.Second))
		...
		t.Run("probabilisticTest", func(t *testing.T) {
			list, _, err := mi.List(ctx, "", "", "", true, 10, meta.None)
...
func TestDeleteExpired(t *testing.T) {
	testplanet.Run(t, testplanet.Config{
		...
		_, cursor, mi := populateTestData(t, planet, time.Now().Add(-time.Second*1))
...
func populateTestData(t *testing.T, planet *testplanet.Planet, expiration time.Time) ([]testData, *audit.Cursor, *metainfo.Service) {
	...
	mi := planet.Satellites[0].Metainfo.Service
	cursor := audit.NewCursor(mi)
	...
	t.Run("putToDB", func(t *testing.T) {
		for _, tt := range tests {
			...
			t.Run(test.bm, func(t *testing.T) {
				...
				require.NoError(t, mi.Put(ctx, test.path, pointer))
			})
		}
	})
	return tests, cursor, mi
}
