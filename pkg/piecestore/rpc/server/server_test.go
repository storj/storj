...
func TestRetrieve(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("should return expected PieceRetrievalStream values", func(t *testing.T) {
			...
			stream.Send(&pb.PieceRetrieval{Bandwidthallocation: &pb.BandwidthAllocation{Signature: []byte{'A', 'B'}, Data: &pb.BandwidthAllocation_Data{Payer: "ABCD", Renter: "ABCD", Size: tt.reqSize}}})
			if err != nil {
				t.Errorf("Unexpected error: %v\n", err)
				return
			}
...
func TestStore(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run("should return expected PieceStoreSummary values", func(t *testing.T) {
			...
			if err = stream.Send(&pb.PieceStore{Bandwidthallocation: &pb.BandwidthAllocation{Signature: []byte{'A', 'B'}, Data: &pb.BandwidthAllocation_Data{Payer: "ABCD", Renter: "EFGH", Size: int64(len(tt.content))}}}); err != nil {
				t.Errorf("Unexpected error: %v\n", err)
				return
			}
