...
func (stream *readonlyStream) segment(ctx context.Context, index int64) (segment storj.Segment, err error) {
	...
	contentKey, err := encryption.DecryptKey(segment.EncryptedKey, stream.Info().EncryptionScheme.CipherSuite, stream.streamKey, &segment.EncryptedKeyNonce)
	if err != nil {
		return segment, err
	}
	...
	if pointer.GetType() == pb.Pointer_INLINE {
		segment.Inline, err = encryption.Decrypt(pointer.InlineSegment, stream.info.EncryptionScheme.CipherSuite, contentKey, nonce)
	} else {
