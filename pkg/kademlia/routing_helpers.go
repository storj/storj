...
// getNodesFromIDs: helper, returns serialized nodes of given nodeIDs
func (rt *RoutingTable) getNodesFromIDs(nodeIDs storage.Keys) ([]storage.Value, error) {
	var nodes []storage.Value
	for _, v := range nodeIDs {
		n, err := rt.nodeBucketDB.Get(v)
		if err != nil {
			return nodes, RoutingErr.New("could not get node id %v, %s", v, err)
		}
...
// unmarshalNodes: helper, returns slice of proto node pointers given a slice of storage values
func unmarshalNodes(nodes []storage.Value) ([]*proto.Node, error) {
...
func (rt *RoutingTable) getUnmarshaledNodesFromBucket(bucketID storage.Key) ([]*proto.Node, error) {
	...
	serializedNodes, err := rt.getNodesFromIDs(nodeIDs)
	if err != nil {
		return []*proto.Node{}, RoutingErr.New("could not get node values %s", err)
	}
	unmarshaledNodes, err := unmarshalNodes(serializedNodes)
	if err != nil {
