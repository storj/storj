...
func (endpoint *Endpoint) Query(ctx context.Context, req *pb.QueryRequest) (_ *pb.QueryResponse, err error) {
	...
	rtLimit := int(req.Limit)
	if rtLimit <= 0 || rtLimit > endpoint.routingTable.bucketSize {
		rtLimit = endpoint.routingTable.bucketSize
	}
	acLimit := int(req.AntechamberLimit)
	if acLimit > endpoint.routingTable.bucketSize {
