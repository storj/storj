		...
		for _, peer := range peers {
			...
			group.Go(func() error {
				for _, target := range peers {
					...
					results, _, err := dialer.Lookup(ctx, &selfnode, peer.Local().Node, target.Local().Node.Id, self.Kademlia.RoutingTable.K(), 0)
					if err != nil {
		...
		for _, target := range targets {
			...
			for _, peer := range peers {
				...
				group.Go(func() error {
					...
					selfnode := self.Local().Node
					results, _, err := dialer.Lookup(ctx, &selfnode, peer.Local().Node, target, self.Kademlia.RoutingTable.K(), 0)
					if err != nil {
						return errs.Combine(errTag, err)
					}
	...
	func() { // Lookup
		...
		for _, peer := range peers {
			...
			group.Go(func() error {
				for _, target := range peers {
					selfnode := self.Local().Node
					_, _, err := dialer.Lookup(ctx, &selfnode, peer.Local().Node, target.Local().Node.Id, self.Kademlia.RoutingTable.K(), 0)
					if !transport.Error.Has(err) || errs.Unwrap(err) != context.DeadlineExceeded {
