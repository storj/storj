...
func newPeerDiscovery(nodes []storj.Node, client node.Client, target storj.NodeID, opts discoveryOptions) *peerDiscovery {
		...
		go func() {
			...
			for {
				...
				for {
					...
					if !lookup.opts.bootstrap && bytes.Compare(next.GetId().Bytes(), lookup.target.Bytes()) == 0 {
						allDone = true
				...
				neighbors, err := lookup.client.Lookup(ctx, next, lookup.target)
				if err != nil {
