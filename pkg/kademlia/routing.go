...
type RoutingTable struct {
	...
	// TODO(bryanchriswhite): wrap kvstore interface with one that uses `NodeID`s and `NodeIDList`s?
	nodeBucketDB     storage.KeyValueStore
	transport        *pb.NodeTransport
	mutex            *sync.Mutex
	replacementCache map[string][]storj.Node
...
func NewRoutingTable(localNode storj.Node, kdb, ndb storage.KeyValueStore) (*RoutingTable, error) {
	rt := &RoutingTable{
		self:             localNode,
		kadBucketDB:      kdb,
		nodeBucketDB:     ndb,
		...
		idLength:         storj.IdentityLength * 8, // NodeID length in bits
...
func (rt *RoutingTable) Local() storj.Node {
	return rt.self
}
...
func (rt *RoutingTable) GetBucket(id storj.NodeID) (bucket dht.Bucket, ok bool) {
	bucketID, err := rt.getKBucketID(id.Bytes())
	if err != nil {
...
func (rt *RoutingTable) FindNear(id storj.NodeID, limit int) ([]storj.Node, error) {
	// if id is not in the routing table
	nodeIDs, err := rt.nodeBucketDB.List(nil, 0)
	if err != nil {
		return []storj.Node{}, RoutingErr.New("could not get node ids %s", err)
	}
	...
	if err != nil {
		return []storj.Node{}, RoutingErr.New("could not get nodes %s", err)
	}
...
func (rt *RoutingTable) ConnectionSuccess(node storj.Node) error {
...
func (rt *RoutingTable) ConnectionFailed(node storj.Node) error {
	key := storage.Key(node.Id.Bytes())
	bucketID, err := rt.getKBucketID(key)
	if err != nil {
		return RoutingErr.New("could not get k bucket %s", err)
	}
	err = rt.removeNode(bucketID, key)
	if err != nil {
...
func (rt *RoutingTable) SetBucketTimestamp(id storj.NodeID, now time.Time) error {
	rt.mutex.Lock()
	defer rt.mutex.Unlock()
	err := rt.createOrUpdateKBucket(id.Bytes(), now)
	if err != nil {
...
func (rt *RoutingTable) GetBucketTimestamp(id storj.NodeID, bucket dht.Bucket) (time.Time, error) {
	t, err := rt.kadBucketDB.Get(id.Bytes())
	if err != nil {
