...
// 	mockBucketGetObject := mock_buckets.NewMockStore(mockBucketCtrl)
// 	b := Storj{bs: mockBucketGetObject}

// 	mockObjGetObject := NewMockStore(mockObjCtrl)
// 	o := buckets.BucketStore{os: mockObjGetObject}
...
func TestGetObjectInfo(t *testing.T) {
	...
	mockObjGetObject := NewMockStore(mockObjCtrl)
	bs := buckets.BucketStore{O: mockObjGetObject}

	BucketMock := storjObjects{storj: &b}
	...
	for i, example := range []struct {
		...
		mockBucketGetObject.EXPECT().GetObjectStore(gomock.Any(), example.bucket).Return(bs.O, nil).Times(1)
		mockObjGetObject.EXPECT().Meta(gomock.Any(), paths.New(example.object)).Return(meta1, example.err).Times(1)

		objInfo, err := BucketMock.GetObjectInfo(context.Background(), example.bucket, example.object)
		if err != nil {
			assert.EqualError(t, err, example.errString)
			if example.err != nil {
				assert.Empty(t, objInfo, errTag)
