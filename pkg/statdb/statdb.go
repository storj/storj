...
type DB interface {
	...
	UpdateAuditSuccess(ctx context.Context, nodeID storj.NodeID, auditSuccess bool) (stats *pb.NodeStats, err error)

	// UpdateBatch for updating multiple farmers' stats in the db
	UpdateBatch(ctx context.Context, updateReqList []*UpdateRequest) (statsList []*pb.NodeStats, failedUpdateReqs []*UpdateRequest, err error)
