...
func testDatabase(ctx context.Context, t *testing.T, sdb statdb.DB) {
	...
	t.Run("TestCreateDoesNotExist", func(t *testing.T) {
		...
		assert.EqualValues(t, nodeID.Bytes(), getResp.GetStats().NodeId.Bytes())
		assert.EqualValues(t, 0, getResp.GetStats().AuditSuccessRatio)
		assert.EqualValues(t, 0, getResp.GetStats().UptimeRatio)
	})
}
