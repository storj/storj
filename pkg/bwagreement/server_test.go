...
func getPeerContext(ctx context.Context, version storj.IDVersion, t *testing.T) (context.Context, storj.NodeID) {
...
func testDatabase(ctx context.Context, t *testing.T, db satellite.DB) {
	testidentity.IdentityVersionsTest(t, func(t *testing.T, version storj.IDVersion, upID *identity.FullIdentity) {
		satID, err := testidentity.IdentityVersions[version.Number].NewIdentity()
		assert.NoError(t, err)
		...
		{ // TestSameSerialNumberBandwidthAgreements
			...
			ctxSN1, storageNode1 := getPeerContext(ctx, version, t)
			rbaNode1, err := testbwagreement.GenerateOrder(pbaFile1, storageNode1, upID, 666)
			assert.NoError(t, err)

			ctxSN2, storageNode2 := getPeerContext(ctx, version, t)
		...
		{ // TestManipulatedBandwidthAgreements
			...
			manipID, err := testidentity.NewTestIdentity(ctx, version.Number)
			assert.NoError(t, err)
