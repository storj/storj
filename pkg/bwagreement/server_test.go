...
func testDatabase(ctx context.Context, t *testing.T, db satellite.DB) {
	...
	{ // TestSameSerialNumberBandwidthAgreements
		...
		assert.NoError(t, err)
		err = testbwagreement.SavePayerBandwidthAllocation(db.UplinkDB(), pbaFile1, upID.Leaf.PublicKey.(*ecdsa.PublicKey))
		assert.NoError(t, err)
		   ...
		   Uplink downloads another file. New PayerBandwidthAllocation with a new sequence. */
		{
			...
			assert.NoError(t, err)
			err = testbwagreement.SavePayerBandwidthAllocation(db.UplinkDB(), pbaFile2, upID.Leaf.PublicKey.(*ecdsa.PublicKey))
			assert.NoError(t, err)
