...
func NewService(queue queue.RepairQueue, config *Config, interval time.Duration, concurrency int, transport transport.Client, pointerdb *pointerdb.Service, allocation *pointerdb.AllocationSigner, statDB statdb.DB, signer signing.Signer, selectionPreferences *statdb.NodeSelectionConfig) *Service {
	return &Service{
		queue:                queue,
		config:               config,
