...
func (o *Server) Lookup(ctx context.Context, req *pb.LookupRequest) (*pb.LookupResponse, error) {
	...
	if err != nil {
		o.logger.Error("Error looking up node", zap.Error(err), zap.ByteString("nodeID", req.NodeId))
		return nil, err
	}

	node, err := o.cache.Get(ctx, nodeID)
	if err != nil {
		o.logger.Error("Error looking up node", zap.Error(err), zap.String("nodeID", nodeID.String()))
		return nil, err
	}
...
func (o *Server) BulkLookup(ctx context.Context, reqs *pb.LookupRequests) (*pb.LookupResponses, error) {
	nodeIDs, err := lookupRequestsToNodeIDs(reqs)
	if err != nil {
		return nil, ServerError.Wrap(err)
	}

	nodes, err := o.cache.GetAll(ctx, nodeIDs)
	if err != nil {
		return nil, ServerError.New("could not get nodes requested %s\n", err)
	}
	return nodesToLookupResponses(nodes), nil
}
...
func (o *Server) FindStorageNodes(ctx context.Context, req *pb.FindStorageNodesRequest) (resp *pb.FindStorageNodesResponse, err error) {
	...
	excluded, err := storj.NodeIDsFromBytes(opts.GetExcludedNodes())
	if err != nil {
	...
	return &pb.FindStorageNodesResponse{
		Nodes: storj.ProtoNodes(result),
	}, nil
}

func (o *Server) getNodes(ctx context.Context, keys storage.Keys) ([]storj.Node, error) {
...
func (o *Server) populate(ctx context.Context,
	starting storage.Key,
	maxNodes,
	restrictedBandwidth,
	restrictedSpace int64,
	excludedIDs storj.NodeIDList) ([]storj.Node, storage.Key, error) {
	...
	if len(keys) <= 0 {
		...
		return []storj.Node{}, starting, nil
	}
...
// containsID checks if a node ID exists in list of string node IDs
func containsID(nodeIDs storj.NodeIDList, nodeID storj.NodeID) bool {
	for _, id := range nodeIDs {
		if bytes.Compare(id.Bytes(), nodeID.Bytes()) == 0 {
			return true
		}
	}
	return false
...
func lookupRequestsToNodeIDs(reqs *pb.LookupRequests) (storj.NodeIDList, error) {
	var idsBytes [][]byte
	for _, v := range reqs.Lookuprequest {
		idsBytes = append(idsBytes, v.NodeId)
	}
	return storj.NodeIDsFromBytes(idsBytes)
}

// nodesToLookupResponses returns LookupResponses from the nodes
func nodesToLookupResponses(nodes []storj.Node) *pb.LookupResponses {
	var rs []*pb.LookupResponse
	for _, v := range nodes {
		r := &pb.LookupResponse{Node: v.Node}
		rs = append(rs, r)
	}
