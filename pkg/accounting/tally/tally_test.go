...
func makeBWA(ctx context.Context, t *testing.T, bwDb bwagreement.DB, serialNum string, fiC *identity.FullIdentity, action pb.PayerBandwidthAllocation_Action) {
	...
	pba, err := bwagreementtest.GeneratePayerBandwidthAllocation(action, fiC, fiC, time.Hour)
	assert.NoError(t, err)
	rba, err := bwagreementtest.GenerateRenterBandwidthAllocation(pba, teststorj.NodeIDFromString("StorageNodeID"), fiC, 666)
	assert.NoError(t, err)
	//save to db
