...
func NewUserRepository(ctx context.Context, db *dbx.DB) accounts.Users {
	return &users{
		db: db,
		ctx: ctx,
	}
...
func (u *users) Get(id uuid.UUID) (*dbo.User, error) {
	...
	return dbo.UserFromDBX(user)
}

// Method for querying user by credentials from the database.
func (u *users) GetByCredentials(password []byte, email string) (*dbo.User, error) {
...
func (u *users) Insert(user *dbo.User) (error) {
...
func (u *users) Update(user *dbo.User) (error) {
